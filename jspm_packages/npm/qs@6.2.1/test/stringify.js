...
(function(Buffer) {
  ...
  test('stringify()', function(t) {
    ...
    t.test('stringifies an object inside an array with dots notation', function(st) {
      st.equal(qs.stringify({a: [{b: 'c'}]}, {
        allowDots: true,
        encode: false
      }), 'a[0].b=c');
      st.equal(qs.stringify({a: [{b: {c: [1]}}]}, {
        allowDots: true,
        encode: false
      }), 'a[0].b.c[0]=1');
      st.end();
    ...
    t.test('drops keys with a value of undefined', function(st) {
      ...
      st.equal(qs.stringify({a: {
          b: undefined,
          c: null
        }}, {strictNullHandling: true}), 'a%5Bc%5D');
      st.equal(qs.stringify({a: {
          b: undefined,
          c: null
        }}, {strictNullHandling: false}), 'a%5Bc%5D=');
      st.equal(qs.stringify({a: {
          b: undefined,
          c: ''
        }}), 'a%5Bc%5D=');
    ...
    t.test('selects properties when filter=array', function(st) {
      ...
      st.equal(qs.stringify({
        ...
        c: 'f'
      }, {filter: ['a', 'b', 0, 2]}), 'a%5Bb%5D%5B0%5D=1&a%5Bb%5D%5B2%5D=3');
      st.end();
    });
    ...
    t.test('can sort the keys', function(st) {
      ...
      st.equal(qs.stringify({
        ...
        b: 'f'
      }, {sort: sort}), 'a=c&b=f&z%5Bi%5D=b&z%5Bj%5D=a');
      st.end();
    });
    ...
    t.test('can use custom encoder for a buffer object', {skip: typeof Buffer === 'undefined'}, function(st) {
      st.equal(qs.stringify({a: new Buffer([1])}, {encoder: function(buffer) {
