...
describe('computeImpliedEntries', function() {
  it('computes implied entries', function() {
    ...
    expect(entries).to.deep.equal([new Entry('a/b/c/', 0, 0), new Entry('a/b/c/d/', 0, 0), new Entry('a/b/c/d/e/', 0, 0)]);
  });
});
describe('sortAndExpand', function() {
  it('sorts and expands entries in place', function() {
    var entries = [new Entry('a/b/q/r/bar.js'), new Entry('a/b/c/d/foo.js')];
    var sortedAndExpandedEntries = sortAndExpand(entries);
    expect(entries).to.equal(sortedAndExpandedEntries);
    expect(sortedAndExpandedEntries.map(function(e) {
      return e.relativePath;
    })).to.deep.equal(['a/', 'a/b/', 'a/b/c/', 'a/b/c/d/', 'a/b/c/d/foo.js', 'a/b/q/', 'a/b/q/r/', 'a/b/q/r/bar.js']);
    expect(sortedAndExpandedEntries).to.deep.equal([new Entry('a/', 0, 0), new Entry('a/b/', 0, 0), new Entry('a/b/c/', 0, 0), new Entry('a/b/c/d/', 0, 0), new Entry('a/b/c/d/foo.js'), new Entry('a/b/q/', 0, 0), new Entry('a/b/q/r/', 0, 0), new Entry('a/b/q/r/bar.js')]);
  });
});
