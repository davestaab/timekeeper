...
module.exports = function(objName) {
  ...
  l = objName.length;
  if (!l)
    throw new TypeError(objName + " is not valid object name");
  return function(code) {
    ...
    esniff(code, objName[0], function(i, previous) {
      ...
      name = '';
      if (!reIdentStart.test(char = code[i]))
        return resume();
      name += char;
      while ((char = code[++i]) && reIdentNext.test(char))
        name += char;
      data.push({
        name: name,
        start: startIndex,
