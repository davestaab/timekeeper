...
(function(process) {
    ...
    return Promise.resolve().then(function() {
      ...
      try {
        if (fs.existsSync(linkDir)) {
          return (force ? Promise.resolve(true) : ui.confirm('`' + pkg.exactName + '` is already linked, are you sure you want to override it (use -y to skip this prompt in future)?', true)).then(function(override) {
            return !override;
          });
        }
      } catch (e) {}
      ...
      if (abort)
        return true;
      return asp(rimraf)(linkDir).then(function() {
  ...
  exports.lookup = function(pkg, edge) {
    ...
    return asp(fs.readdir)(linkFolder).then(function(files) {
      var versions = {};
      var hasVersions;
  ...
  exports.symlink = function(pkg, downloadDeps) {
    ...
    var pjson;
    var fresh = false;
    return asp(fs.readlink)(dir).then(function(linkString) {
      if (linkString === linkDir)
        return asp(fs.unlink)(dir);
      return Promise.resolve(ui.confirm('`' + pkg.exactName + '` already linked, are you sure you want to link over it?', true)).then(function(remove) {
        if (!remove)
          throw 'Aborted.';
        return asp(fs.unlink)(dir);
      });
      ...
      if (err.code !== 'EINVAL' && err.code !== 'UNKNOWN')
        throw err;
      return Promise.resolve(ui.confirm('`' + pkg.exactName + '` already installed, are you sure you want to link over it?', true)).then(function(remove) {
        if (!remove)
          throw 'Aborted.';
        return asp(rimraf)(dir);
      });
    }).then(function() {
      return asp(mkdirp)(path.resolve(dir, '..')).then(function() {
