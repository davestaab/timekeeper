...
(function(Buffer, process) {
  ...
  var compareSourceMaps = function(filename, expectation, done, transpiler) {
    ...
    instance.bundle(filename, null, buildOpts).then(function(output) {
  ...
  function writeTestOutput(done) {
    ...
    return builder.loadConfig(configFile).then(function() {
  ...
  function writeSourceMaps(moduleName, transpiler, sourceMapFile) {
    ...
    buildOpts.config = buildOpts.config || {};
    buildOpts.config.transpiler = transpiler || 'traceur';
    instance.bundle(moduleName, null, buildOpts).then(function(output) {
  ...
  suite('Source Maps', function() {
    ...
    test('can render inline', function(done) {
      ...
      var instance = new Builder();
      instance.loadConfigSync(configFile);
      instance.bundle(module, null, {sourceMaps: 'inline'}).then(function(output) {
    ...
    test('can consume input source maps', function(done) {
      ...
      var instance = new Builder();
      instance.loadConfigSync(configFile);
      instance.bundle(module, null, {sourceMaps: true}).then(function(output) {
        var sources = getSources(output.sourceMap);
        assert.deepEqual(sources, ["test/fixtures/test-tree/jquery.js", "test/fixtures/test-tree/global.js", "test/fixtures/test-tree/example", "test/fixtures/test-tree/babel.js"]);
      }).then(done).catch(err);
    });
    test('can be disabled for tracing', function(done) {
      ...
      var instance = new Builder();
      instance.loadConfigSync(configFile);
      instance.bundle(module, {sourceMaps: false}).then(function(output) {
    ...
    suite('sources paths', function() {
      test('are relative to outFile', function(done) {
        ...
        builder.buildStatic('first.js', 'test/output/output.js', buildOpts).then(function(outputs) {
          var sources = getSources(outputs.sourceMap);
          assert.deepEqual(sources, ['../fixtures/test-tree/third.js', '../fixtures/test-tree/cjs.js', '../fixtures/test-tree/second.js', '../fixtures/test-tree/jquery.js', '../fixtures/test-tree/global.js', '../fixtures/test-tree/text.txt', '../fixtures/test-tree/amd.js', '../fixtures/test-tree/unknown', '../fixtures/test-tree/file.json', '../fixtures/test-tree/first.js']);
        }).then(done).catch(err);
      });
      test('are relative to baseURL, if no outFile', function(done) {
        ...
        builder.buildStatic('first.js', null, opts).then(function(outputs) {
          var sources = getSources(outputs.sourceMap);
          assert.deepEqual(sources, ['third.js', 'cjs.js', 'second.js', 'jquery.js', 'global.js', 'text.txt', 'amd.js', 'unknown', 'file.json', 'first.js']);
        }).then(done).catch(err);
      });
    });
