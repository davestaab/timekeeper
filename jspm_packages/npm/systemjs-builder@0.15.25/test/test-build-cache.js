...
suite('Test compiler cache', function() {
  ...
  test('builder.loader.fetch sets load.metadata.timestamp', function() {
    ...
    return builder.loader.fetch(load).then(function(text) {
      assert(text == source);
      assert(load.metadata.deps);
      assert(load.metadata.timestamp);
    });
  });
  ...
  test('Static build, fetch override with callback', function() {
    var builder = new Builder('test/fixtures/test-tree');
    return builder.buildStatic('cjs.js', {fetch: function(load, fetch) {
        return fetch(load);
      }});
  });
