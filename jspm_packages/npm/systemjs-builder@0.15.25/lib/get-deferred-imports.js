...
DeferredImportsTransformer.prototype.transformCallExpression = function(tree) {
  ...
  if (tree.operand.type == 'MEMBER_EXPRESSION' && tree.operand.memberName.value == 'import' && tree.operand.operand.type == 'IDENTIFIER_EXPRESSION' && tree.operand.operand.identifierToken.value == 'System' && tree.args.args.length == 1) {
...
module.exports = function(builder, trace) {
  ...
  return Promise.all(Object.keys(trace).map(function(name) {
    ...
    return Promise.all(transformer.imports.map(function(impt) {
      return builder.loader.normalize(impt).then(function(moduleName) {
