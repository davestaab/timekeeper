...
(function(process) {
  ...
  function parseExpression(expressionString) {
    ...
    function getNextOperator() {
      ...
      if (index === expressionString.length)
        return null;
      var candidateResult = expressionString.charAt(index++);
  ...
  exports.traceExpression = function(builder, expression, traceOpts) {
    ...
    return Promise.resolve(expandAndCanonicalizeExpression(builder, expression)).then(function processExpandedOperations(expandedOperations) {
  ...
  function expandAndCanonicalizeExpression(builder, expression) {
    ...
    operations.forEach(function(operation) {
      if (operation.bulkOperation) {
        ...
        expandPromise = expandPromise.then(function() {
          return Promise.resolve(expandedTreePromise).then(function(expressionsOperations) {
            expandedOperations = expandedOperations.concat({
              operator: operation.operator,
              operationsTree: expressionsOperations
            });
          });
        });
      } else {
    ...
    return Promise.resolve(expandPromise).then(function() {
