...
describe('lib/auth', function() {
  ...
  describe('injectRequestOptions', function() {
    var baseRequestOptions = {url: 'https://registry.npmjs.org/'};
    var requestOptions;
    beforeEach(function() {
    ...
    it('should inject BasicAuth options into request options', function() {
      ...
      return expect(requestOptions, 'to satisfy', {auth: {
          user: 'foo',
          pass: 'bar'
        }});
    });
    it('should inject auth tokens into request options', function() {
      requestOptions = auth.injectRequestOptions(requestOptions, {token: 'foobar'});
      return expect(requestOptions, 'to satisfy', {headers: {authorization: 'Bearer foobar'}});
    });
    it('should inject auth tokens into request options with existing headers', function() {
      requestOptions.headers = {'X-Custom-Header': 'helloworld'};
      requestOptions = auth.injectRequestOptions(requestOptions, {token: 'foobar'});
      return expect(requestOptions, 'to satisfy', {headers: {
  ...
  describe('configureCredentials', function() {
    it('token based authentication chosen', function() {
      ...
      return expect(credentialsPromise, 'to be fulfilled with', {'token': 'foobar'});
    });
    it('username and password authentication chosen, opt out of validation', function() {
      var ui = uiFactory([{
        ...
        method: 'confirm',
        message: /test these credentials/,
        returns: false
      }]);
      var credentialsPromise = auth.configureCredentials('http://registry.npmjs.org', null, ui);
    ...
    it('using an already provided token', function() {
      ...
      var credentialsPromise = auth.configureCredentials('http://registry.npmjs.org', {token: 'foobar'}, ui);
      return expect(credentialsPromise, 'to be fulfilled with', {token: 'foobar'});
    });
    ...
    it('username and password authentication chosen, validation working', function() {
      var ui = uiFactory([{
        ...
        method: 'log',
        message: 'npm authentication is working successfully.'
      }]);
      var credentialsPromise = auth.configureCredentials('http://registry.npmjs.org', null, ui);
      return expect(credentialsPromise, 'with http mocked out', {
        request: {
          url: 'GET http://registry.npmjs.org/',
          headers: {Authorization: 'Basic c3VwZXJtYW46Y2xhcmsrbG91aXM='}
        },
        response: 200
    ...
    it('username and password authentication chosen, validation temporarily failing, deny retrying', function() {
      var ui = uiFactory([{
        ...
        method: 'log',
        message: 'Invalid response code, %503%'
      }, {
        method: 'confirm',
        message: 'Would you like to try new credentials?',
        returns: false
      }]);
      var credentialsPromise = auth.configureCredentials('http://registry.npmjs.org', null, ui);
      return expect(credentialsPromise, 'with http mocked out', {
    ...
    it('username and password authentication chosen, validation permanently failing, deny retrying', function() {
      var ui = uiFactory([{
        ...
        method: 'log',
        message: 'Provided npm credentials are not authorized.'
      }, {
        method: 'confirm',
        message: 'Would you like to try new credentials?',
        returns: false
