...
Em.$(document).on('focusin', handleTabIntoBrowser);
...
export default Em.Component.extend({
  ...
  open: function(options) {
    ...
    Em.run.schedule('afterRender', this, function() {
      ...
      if (options.focus !== false) {
        ...
        Em.run.schedule('afterRender', this, 'focus');
      } else {
  ...
  /**
   * Often you need a mechanism besides an modal-toggle to close an
   ...
   *
   * ```html
   * {{#modal-container close-when=formIsSubmitted}}
   ...
   * `modal-title` or an `modal-toggle` then this will create some
   * defaults.
   *
